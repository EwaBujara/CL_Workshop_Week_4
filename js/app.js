...
function addBookDetailsEvent(){
    let listEl = document.getElementById("bookList");
    listEl.addEventListener("click", function foo(e) {
        let caller = e.target;
        if(caller.tagName === "H2"){
            let bookId = caller.parentElement.dataset.id;

            getBookDetails(bookId, function(book){
              let divElem = Array.from(caller.children)[1];
              divElem.innerHTML="isbn: "+book.isbn +"<br>"+
                  "Author: "+book.author +"<br>"+
                  "Publisher: "+book.publisher+"<br>"+
                  "Type: "+book.type;

                divElem.classList.toggle("hide");
            });
